...
languages = {
    "ru": {
        ...
        "register_info": f"""🔥<b>Чтобы получить максимум от использования этого бота, необходимо следовать следующим шагам:</b> 

1. <b>Зарегистрируйте новый аккаунт!</b> 
[Если у вас уже есть аккаунт, пожалуйста, выйдите и зарегистрируйте новый]

2. <b>Используйте промокод</b> 👉🏻{PROMO}👈🏻 <b>при регистрации нового аккаунта</b>. 
...
""",
    ...
    "en": {
        ...
        "register_info": f"""🔥<b>To get the most out of using this bot, you need to follow these steps:</b> 

1. <b>Register a new account!</b> 
[If you already have an account, please log out and register a new one]

2. <b>Use the promo code</b>  👉🏻{PROMO}👈🏻 <b>when registering the new account</b>. 
...
🟢 6. In case of an unsuccessful signal, we recommend doubling (x²) your bet to completely cover the loss with the next signal.""",
    ...
    "hi": {
        ...
        "register_info": f"""🔥<b>इस बॉट का अधिकतम लाभ उठाने के लिए, आपको इन चरणों का पालन करना होगा:</b>

1. <b>एक नया खाता रजिस्टर करें!</b> 
[यदि आपके पास पहले से ही एक खाता है, तो कृपया लॉग आउट करें और एक नया खाता रजिस्टर करें]

2. <b>नए खाते को रजिस्टर करते समय प्रोमो कोड का उपयोग करें</b> 👉🏻{PROMO}👈🏻।
...
""",
    ...
    "es": {
        ...
        "register_info": f"""🔥<b>Para aprovechar al máximo el uso de este bot, necesitas seguir estos pasos:</b>

1. <b>¡Registra una nueva cuenta!</b> 
[Si ya tienes una cuenta, cierra la sesión y registra una nueva]

2. <b>Utiliza el código promocional</b> 👉🏻{PROMO}👈🏻 <b>al registrar la nueva cuenta</b>.
...
""",
    ...
    "az": {
        ...
        "register_info": f"""🔥<b>Bu botdan maksimum yararlanmaq üçün aşağıdakı addımları yerinə yetirməlisiniz:</b> 
...
2. <b>Yeni hesabı qeydiyyatdan keçirərkən promo koddan istifadə edin</b> 👉🏻{PROMO}👈🏻.
...
""",
    ...
    "pt": {
        ...
        "register_info": f"""🔥<b>Para aproveitar ao máximo o uso deste bot, siga estas etapas:</b> 

1. <b>Registre uma nova conta!</b> 
[Se você já possui uma conta, por favor, saia e registre uma nova]

2. <b>Use o código promocional</b> 👉🏻{PROMO}👈🏻 <b>ao registrar a nova conta</b>.
...
""",
    ...
    "tr": {
        ...
        "register_info": f"""🔥<b>Bu botu en iyi şekilde kullanmak için aşağıdaki adımları takip etmelisiniz:</b> 

1. <b>Yeni bir hesap kaydedin!</b> 
[Eğer zaten bir hesabınız varsa, lütfen çıkış yapın ve yeni bir hesap oluşturun]

2. <b>Yeni hesap oluştururken promosyon kodunu kullanın</b> 👉🏻{PROMO}👈🏻.
...
[Açılmıyorsa, VPN (İsveç) etkinleştirilmiş olarak erişin. Play Market/App Store'da bir çok ücretsiz hizmet bulunmaktadır, örneğin: Vpnify, Planet VPN, Hotspot VPN vb.!]
...
""",
    ...
    "ar": {
        ...
        "welcome_message": """<b>Əsas menyu:</b>""",
        "register_info": f"""🔥<b>للحصول على أقصى استفادة من استخدام هذا الروبوت، يجب عليك اتباع الخطوات التالية:</b>

1. <b>سجّل حسابًا جديدًا!</b> 
[إذا كان لديك حساب بالفعل، يرجى تسجيل الخروج وإنشاء حساب جديد]

2. <b>استخدم رمز العرض الترويجي</b> 👉🏻{PROMO}👈🏻 <b>عند تسجيل الحساب الجديد</b>.
...
""",
    ...
    "uz": {
        ...
        "register_info": f"""🔥<b>Ushbu botdan maksimal darajada foydalanish uchun quyidagi bosqichlarni bajaring:</b> 
...
2. <b>Yangi qaydnomani ro'yxatdan o'tkazishda promo kodni kiriting</b> 👉🏻{PROMO}👈🏻.
